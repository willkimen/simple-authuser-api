...
def test_task_send_activation_code_by_email_success(deactivated_user):
    """
    Test the successful execution of the task_send_activation_code_by_email task.
    ...
    actual_sent_count = task_send_activation_code_by_email(deactivated_user.email)
    assert expected_success_send_email == actual_sent_count


@pytest.mark.django_db
@patch(
    f"{TASKS_MODULE_PATH}.{SEND_ACTIVATION_CODE_BY_EMAIL_FUNCTION_TO_PATCH}",
    side_effect=SMTPException(),
)
@patch.object(task_send_activation_code_by_email, "retry", side_effect=Retry())
def test_task_send_activation_code_by_email_failure(
    mock_retry: MagicMock, mock_send_activation: MagicMock, deactivated_user
):
    """
    Test the failure scenario for the task_send_activation_code_by_email task.

    This test simulates an SMTPException being raised when the
    send_activation_code_by_email function is called. It then verifies that
    ...
    with pytest.raises(Retry):
        task_send_activation_code_by_email(deactivated_user.email)
