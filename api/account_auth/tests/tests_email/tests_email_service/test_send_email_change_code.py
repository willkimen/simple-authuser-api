...
import pytest
from account_auth.models import ChangeEmailCodeModel
...
from account_auth.email.email_service import send_email_change_code
...
def test_success_send_email(deactivated_account):
    ...
    **Expected Results**:
    ...
    actual_sent_count = send_email_change_code(deactivated_account.email, NEW_EMAIL)
...
def test_success_send_email_create_code_in_database(deactivated_account):
    ...
    """
    send_email_change_code(deactivated_account.email, NEW_EMAIL)

    assert ChangeEmailCodeModel.objects.filter(account_id=deactivated_account.email).exists()
    assert (
        ChangeEmailCodeModel.objects.filter(account_id=deactivated_account.email).count() == 1
    )
